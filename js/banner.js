	...
	var image;		//获取图片src节点
	var current=1;
	var nav_current_left = 126; //定义边框current左边距
	...
	function init(){
		...
		for(i =1 ;i<=5 ;i++){
			...
			images[i].src = "images/banner"+i+".jpg";	//给数组里面的每个Image对象的src属性赋值
		};
		...
		titles[5] = "Ceramic Lined Equipments";
		
		//描述内容初始化：txts容器
		txts[1] = "Kingcera can provides domestic & international inatallation, technical and maintenance support";
		...
		txts[5] = "Kingcera provides the customer base cost-effective,high quality and customized wear resistant ceramic equipments for different industries";
	...
	function setSrcLeft(p){
		...
		$('.describe_bg').animate({top:'422px'},10)
			.delay(1500)
			.animate({top:'285px'},1600);
	
		
//		$('.describe_bg').fadeToggle()
//		.delay(1500)
//		.fadeToggle(3000);
	};
	...
	window.onload = (timeID=setInterval('next()',5000));
